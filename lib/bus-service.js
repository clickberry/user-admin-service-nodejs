...
function Bus(options) {
    ...
    var profileUpdateReader=new nsq.Reader('profile-updates', 'user-admin', options);
    ...
    profileUpdateReader.connect();
    profileUpdateReader.on('message', function (message) {
        var e = {
            profile: message.json(),
            message: message
        };

        bus.emit('profile-update', e);
    });
}
